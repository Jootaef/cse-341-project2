...
const getSingle = async (req, res) => {
  try {
    ...
    const result = await db.getDatabase().collection('items').findOne({ _id: itemId });
...
const createItem = async (req, res) => {
  ...
  try {
    const response = await db.getDatabase().collection('items').insertOne(newItem);
...
const updateItem = async (req, res) => {
  ...
  try {
    const itemId = new ObjectId(req.params.id);
    const result = await db.getDatabase().collection('items').replaceOne({ _id: itemId }, updatedItem);
    if (result.modifiedCount === 0) return res.status(404).json({ message: 'Item not found' });
